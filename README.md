...
- Директория `./artifacts` содержит отчеты, графики и прочее, полученное в процессе выполнения задания. 
- Проект `reference-app` является целевым, на который по заданию необходимо было ориентироваться в процессе выполнения. 
- Директория `./scripts/` содержит скрипты, использованные для автоматизации отчетов.
- Проект `broken-app` - основной проект, где проводились отладка, профилирование и оптимизации.
